...
REQ_packages <- c("stringr", "dplyr", "ggplot2", "tidyr", "readr", "optparse", "this.path", "ggalt", "remotes")


#CRAN and GitHub packages
GH_packages <- c("ggalt")
CRAN_packages <- c("stringr", "dplyr", "ggplot2", "tidyr", "readr", "optparse", "this.path", "remotes")

GH_repo <- c("hrbrmstr/ggalt")

# A function to check and install packages
package_controller = function(packages = REQ_packages) {
    ...
    if (!is.null(packages)) {
        ...
        for (element in packages) {
            ...
            if (!is.element(element, installed.packages())) {
                ...
                #The used packages are from two main source: CRAN and GitHub, so if the package is not on CRAN it is on github
                if (is.element(element, CRAN_packages)) {
                  install.packages(element)

                } else if (is.element(element, GH_packages)) {
...
package_loader = function(packages = REQ_packages) {
    for (pkg in packages) {
...
options_list <- list(
    ...
    from measurement to measurement. It is important, that the treatment_variation values should be negative or close to 0 to mimic growth arrest or shrinkage.
...
prog_descr <- c(paste0("\nSynthXenoGen (Synthetic Xenograft Data Generator) is a CLI (Command Line Interface) tool designed to generate synthetic sample data for the XenoVol script.
It produces Length and Width measurements (mimicking caliper measurements), LxW measurement products, and full volume measurements (mimicking µCT measurements) and the associated
growth curve plots. Note that the total volume calculation assumes an ellipsoid shape.
The author hopes this tool will also be useful for other forms of simple xenograft growth modeling under various control and treatment conditions.

SynthXenoGen simulates the growth of subcutaneously injected tumor cells, forming tumors on the flanks of the injected animals.
The model supports both exponential and logistical tumor growth patterns and simulates treatment responses, including continuous or single-dose treatments.
To execute the tool, use the syntax:
'Rscript SynthXenoGen_v[option] [optional args]'. \n",
...
tumor_data_generator = function(number_of_measurements, number_of_samples, growth_model, vol_calc, carrying_capacity, base_intrinsic_growth_rate,
    ...
    ##Generate sample data based on exponential or logistic growth
    ...
    if (growth_model == "exponential") {
        ...
        for (element in seq_len(nSamples)) {
            ...
            for (dim in dimensions) {
                ...
                #This while loop ensures that the initial element is more than or equal to 0.5 (to model some level of tumor growth to the first measurement
