...
calculate_f_constants = function(bind_and_unify_measurements_output_list) {
    ...
    for (i in seq_along(bind_and_unify_measurements_output_list)) {
        ...
        for (e in seq_len(nrow(bind_and_unify_measurements_output_list[[i]]))) {
            ...
            temp_values <- as.numeric(unlist(bind_and_unify_measurements_output_list[[i]][e, c(grep(pattern = "Volume", x = colnames(bind_and_unify_measurements_output_list[[i]])))]))

            #Calculate the mean, ignoring NA values
            temp_mean <- mean(temp_values, na.rm = TRUE)
            
            measurements_with_f_constants_list[[i]]$f_constants[e] <- temp_mean / ((pi / 6) * (bind_and_unify_measurements_output_list[[i]]$L[e] * bind_and_unify_measurements_output_list[[i]]$W[e])^(3 / 2))
