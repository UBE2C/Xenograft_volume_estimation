...
clean_uCT_data = function(read_uCT_output_df) {
    #Remove entries without corresponding caliper measurements
    clean_uCT_df <- dplyr::filter(.data = read_uCT_output_df, Has_corresponding_caliper_measurements == "Yes")
    
    #Resets the row numbers
    rownames(clean_uCT_df) <- seq_len(nrow(clean_uCT_df))

    #Return the clean uCT data frame
    return(clean_uCT_df)
}
