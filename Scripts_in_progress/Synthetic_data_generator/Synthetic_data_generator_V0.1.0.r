...
options_list <- list(
    ...
    (Note: this option only work with formatted data!)
    By default the option is set to TRUE."),
...
prog_descr <- c(paste0("This CLI tool was originally created to generate synthetic sample data to the Tumor_volume_estimator script,",
"Based on the use of the f-constant formula (Sápi et al. 2015). \nThis tool is set to mimic the growth of subcutaneously injected LM8-osteosarcoma cells, \n",
"forming tumors on the flank of the animals. The model is set to mimic linear tumor growth and linear treatment response. \n",
"For a standalone run use the 'Rscript Synthetic_data_generator_V[option] [optional args]' syntax. \n",
"Author: Gábor Bakos"))
...
tumor_data_generator = function(number_of_measurements, number_of_samples, request_dates, dates, seed, initial_element_mean,
    initial_element_sd, growth_variation_min, growth_variation_max, treatment_administered, treatment_effect_start, 
...
plot_tumor_volumes = function(data, number_of_measurements, number_of_samples, dates) {
...
main = function(measurements = arguments$number_of_measurements, samples = arguments$number_of_samples, rdates = arguments$dates, sdate = arguments$start_date,
    bydays = arguments$by_days, set_seed = arguments$seed, init_element_mean = arguments$initial_element_mean, init_element_sd = arguments$initial_element_sd,
    gr_variation_min = arguments$growth_variation_min, gr_variation_max = arguments$growth_variation_max, treatment = arguments$treatment,
    treatment_start = arguments$treatment_start, tr_variation_min = arguments$treatment_variation_min, tr_variation_max = arguments$treatment_variation_max,
    ...
    plot = arguments$plot, save_pl = arguments$save_plots, output_path = arguments$output_path) {
        ...
        Synthetic_data <- tumor_data_generator(number_of_measurements = measurements, number_of_samples = samples, request_dates = rdates,
            dates = Dates, seed = set_seed, initial_element_mean = init_element_mean, initial_element_sd = init_element_sd, growth_variation_min = gr_variation_min,
            growth_variation_max = gr_variation_max, treatment_administered = treatment, treatment_effect_start = treatment_start,
            treatment_variation_min = tr_variation_min, treatment_variation_max = tr_variation_max, continuous_treatment = cont_treatment)

        
        ##Call the format data subfunction if requested
        if (format == TRUE) {
            message("Data formatting was requested. This also allows the generation of plots. \n")
