...
plot_tumor_volumes = function(data, number_of_measurements, number_of_samples) {
    ...
    ##Pivot the input dataframe for visualization
    long_df <- tidyr::pivot_longer(input_dataframe, cols = 4:ncol(input_dataframe), names_to = "Dates", values_to = "Volumes")
    
    
    ## Plot the resulting unified dataframes
    Volume_plot <- ggplot2::ggplot(data = long_df,
                    mapping = aes(x = Dates, y = Volumes, fill = Mouse_ID, color = Mouse_ID)) +
