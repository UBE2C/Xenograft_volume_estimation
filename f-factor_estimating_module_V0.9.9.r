...
prog_descr <- c(paste0("\nThis CLI (Command Line Interface) tool was created to help estimate tumor xenograft volumes the f-constant formula described by
Feldman JP el al. (2010) and by Sápi et al. (2015). The main intention for this tool is to automate the calculation process for 
the volume calculation especially in the case of tumors where the height measurements are difficult or impossible however reference
µCT measurements are available (such as xenografts growing on the flank).
For a standalone run use the 'Rscript Xenograft_volume_estimator_V[option] [optional args]' syntax."))
...
model_precision_test = arguments$precision_test, volume_corr = arguments$volume_correction, correction_method = arguments$final_correction_method) {
    ...
    if (rm_na_samples == TRUE) {
        ...
        if (verb == TRUE) {
            cat("As the NA value containing samples were not removed during data cleaning, the goodnes of fit test are less reliable in case of the corrected volumes. \n.",
    ...
    ##Implement a model precison test to see the estimation errors with or without volume correction
